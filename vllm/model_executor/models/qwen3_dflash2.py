...
def _lm_head_supports_topk(lm_head) -> bool:
    """Whether the head's dispatch can serve the candidate TopK.
...
class DFlash2Qwen3ForCausalLM(DFlashQwen3ForCausalLM):
    ...
    def compute_candidates(
        ...
        if not _lm_head_supports_topk(self.lm_head):
            raise ValueError(
                "DFlash2 requires an unquantized target LM head for candidate "
                "TopK, got quant method "
